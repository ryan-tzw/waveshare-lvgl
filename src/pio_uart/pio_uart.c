...
static void pio_uart_irq_handler(void) {
    for (size_t i = 0; i < PIO_UART_MAX_INSTANCES; ++i) {
        ...
        while (!pio_sm_is_rx_fifo_empty(uart->rx_pio, uart->rx_sm)) {
            uint8_t byte =
                (uint8_t)uart_rx_program_getc(uart->rx_pio, uart->rx_sm);
...
static bool pio_uart_configure_irq(
    PIO pio,
    uint *irq_index,
    int *irq_num
) {
    ...
    for (uint candidate = 0; candidate < NUM_PIO_IRQS; ++candidate) {
        ...
        if (irq_get_exclusive_handler((uint)candidate_irq) != NULL) {
            continue;
        }
...
bool pio_uart_init(PioUart *uart, uint tx_pin, uint rx_pin, uint baud_rate) {
    if (uart == NULL || uart->initialized || baud_rate == 0) {
        return false;
    }
...
void pio_uart_write_byte(PioUart *uart, uint8_t byte) {
    ...
    uart_tx_program_putc(uart->tx_pio, uart->tx_sm, (char)byte);
}
